...
- **`pkg/` is the shared surface; `internal/` is everything else.** `pkg/{openalex,citekey,doi,doiorg}` are imported by sibling SciMinds repos (today: `zotero-mcp`), so their exported API is a compatibility commitment — a breaking change there needs a `!` commit and a heads-up to the consumer. Keep them **stdlib-only**: a third-party dep in `pkg/` propagates into every consumer's module graph. Everything else stays in `internal/`, including `zot/api` and `zot/local` — those are the Zotero *operate* plane and are deliberately not shared. `just ok` covers `./pkg/...` everywhere it covers `./internal/...` (vet, lint, lint-docs, semgrep, lint-guard rule 12).
