...
- **Config storage:** per-domain config files live under the sci config dir (`~/.config/sci/<name>.json`) via `internal/sciconfig`. Declare `var configFile = sciconfig.File[Config]{Name: "<name>.json"}` and delegate `ConfigPath`/`Load`/`Save`/`Exists`/`Clear` to it — don't re-roll the XDG-empty-string fallback, JSON marshal, or `0600` write. Domain logic (validation, schema migration via `LoadRaw`, defaulting) layers on top. The top-level `sci setup` (`cmd/sci/setup.go`) is a hub/menu router over each domain's existing setup flow; register a new tool by adding a `setupEntry` (status fn + delegating run fn) — it does not reimplement setup. `cass` (split global token + project `cass.yaml`) and `cloud` (HF auth, no sci file) are not yet on sciconfig/`sci setup` (tracked as follow-up work).
...
- **`uikit` owns `huh` — no other package imports it.** Two tiers: (1) **single prompts** use the wrappers `uikit.Input` / `uikit.InputInto` / `uikit.Select` / `uikit.MultiSelect`, which expose `uikit.Option` / `uikit.NewOption` / `uikit.WithPassword` / `uikit.WithValidation` so no caller needs `charm.land/huh/v2`; (2) **multi-field forms** (several fields on one screen, optionally conditional) use the builder — `uikit.NewForm(uikit.FormGroup(uikit.FormInput / uikit.FormSelect …)).Run()`, with `FormGroup(...).HideWhen(cond)` for conditional/multi-page groups and `uikit.WithDescription` for per-field help text. Confirmations use `cmdutil.Confirm` / `cmdutil.ConfirmYes` / `cmdutil.ConfirmRequired` (backed by `uikit.Confirm`). Enforced by lint-guard **rule 15** (huh import banned anywhere outside `internal/uikit/`); rule 14 keeps even uikit-internal execution funneled through `runHuhForm` (theme + keymap + stdin drain).
