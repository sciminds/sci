...
- **`uikit` owns `huh` — callers must not import it.** Two tiers: (1) **single prompts** use the wrappers `uikit.Input` / `uikit.InputInto` / `uikit.Select` / `uikit.MultiSelect`, which expose `uikit.Option` / `uikit.NewOption` / `uikit.WithPassword` so no caller needs `charm.land/huh/v2`; (2) **multi-field forms** (several fields on one screen — uikit has no builder for these yet) are the one escape hatch: build a `*huh.Form` and run it through `uikit.RunForm`, the *only* sanctioned reason to import huh outside `uikit`. Never call `.Run()` on a huh form directly — `RunForm` handles theme, keymap, and stdin drain. Confirmations use `cmdutil.Confirm`/`cmdutil.ConfirmYes`. Enforced by lint-guard rules 14 (no raw `.Run()`) and 15 (huh import only with `RunForm`).
