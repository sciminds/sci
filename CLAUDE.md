...
- `just lint-guard` — import boundaries, flag conventions, API rules (`scripts/lint-guard.sh`, numbered). Highlights: rule 9 bans the legacy `sort` package (use `slices.Sort`/`SortFunc`/`SortStableFunc`/`BinarySearch`); rule 13 requires every new `huh` prompt / TUI launch to be registered in `scripts/scriptable-inventory.yml` (hard gate, hand-maintained YAML); rule 14 bans raw `huh` `.Run()` (use `uikit.RunForm`); rule 15 bans `huh` imports outside `internal/uikit/`; rule 16 requires every `api.ItemPatch` that carries `Version` to declare a `Rebuild` hook (a Zotero PATCH replaces whole arrays, so answering a 412 by resubmitting a locally-derived payload is silent data loss — see `internal/zot/CLAUDE.md`); rule 17 is the inverse of rule 4 — a `*SliceFlag` must **not** carry `Local: true`, because urfave/cli truncates a `Local` slice flag to its last occurrence (mark it `// lint:no-local` instead); rule 18 is the zot-namespace boundary in three halves — a `go list -deps` fence keeping it off every metered upstream index, a second keeping `net/http` inside `internal/zot/{api,cli,client,connector}`, and (since 2026-08-13, when the last Web-API write left) a method-level grep asserting nothing outside `internal/zot/api` calls a Zotero WRITE, with a classification gate in front so the derived write list cannot go stale silently (see `internal/zot/CLAUDE.md`, "The boundary").
...
| `internal/zot/` (Zotero local read surface + hygiene) | `internal/zot/CLAUDE.md` |
...
- **CLI:** urfave/cli v3; all flags `Local: true` — *except* slice flags, which corrupt under `Local` and are gate-rejected by lint-guard rule 17 (waiver + reason in `internal/zot/CLAUDE.md`). A slice flag whose values are human names or free text also needs `DisableSliceFlagSeparator` on its command, or urfave splits them on comma.
...
- **SQLite:** pure Go (`modernc.org/sqlite`), no CGO. Canonical store at `internal/store/sqlite/` (raw `database/sql`; used by `sci db`, `sci view`, dbtui). `pkg/local/` keeps its own connection (read-only immutable mode on `zotero.sqlite`).
...
- **`pkg/` is the shared surface; `internal/` is everything else.** `pkg/{openalex,crossref,doi,doiorg,citekey,local}` are imported by the sibling `zot` repo, so their exported API is a compatibility commitment — a breaking change there needs a `!` commit and a heads-up to the consumer. **Several of them now have no sci-side importer at all** (`openalex`, `crossref`, `doiorg`, since sci's Zotero surface shrank to the local read plane) and stay anyway: they are zot's dependency, and `pkg/` is the seam. **Their dependency weight is part of the commitment**, because everything a `pkg/` package imports propagates into every consumer's module graph. Measure it with `go list -deps ./pkg/<name>` before adding an import; `doi` and `doiorg` are stdlib-only, `openalex` adds `samber/lo` + `x/text`, and `local` — the hardened `zotero.sqlite` reader, promoted so the operate binary can share it — carries `modernc.org/sqlite` plus `internal/tui/dbtui/match` (a `pkg/` package importing its own module's `internal/` is legal Go, but the weight still propagates); `citekey` rides on `local`. Everything else stays in `internal/`, including `zot/api` — the Zotero Web API client, deliberately not shared. `just ok` covers `./pkg/...` everywhere it covers `./internal/...` (vet, lint, lint-docs, semgrep, lint-guard rule 12).
