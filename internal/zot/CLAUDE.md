...
- **BibTeX export** (`export.go` + `exportlib.go` + `citekey.go`): single-item writer `writeBibEntry` is shared by `zot item export` and the library exporter `ExportLibrary`. Cite-keys resolve via `ResolveCiteKey` — native Zotero 7 `citationKey`, then legacy BBT `Citation Key:` in `extra`, then synthesized `lastname{year}{firstword}-ZOTKEY` (see citekey.go for the stopword list / ASCII-fold). Synthesized keys use the 8-char Zotero key suffix as a uniqueness/round-trip anchor — no collision arithmetic. Pinned entries append `zotero://select/library/items/<KEY>` to the `note` field; user-authored `extra` prose is preserved via `extractExtraNote` (strips `Citation Key:` lines, keeps everything else). Drift detection: `ExportLibrary` takes a prior `Keymap` and emits biblatex `ids = {oldkey}` when a synthesized prefix changed between runs. `cli/export.go` persists `.zotero-citekeymap.json` next to `-o FILE` and skips the sidecar write when `len(stats.Keymap) == 0` to avoid clobbering an existing file from a different export. Only `{`/`\` are escaped — for full LaTeX escaping users should use Better BibTeX's own export.
