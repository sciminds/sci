...
- **`internal/zot/api`** — the `--remote` live reads (`item read`/`list`/`children`, `collection list`, `search`, `link list`/`suggest`), the live-only reads (`item note read`/`list`, `saved-search list`/`show`), and the `link` relation writes. The user's own key against the user's own library.
...
**`link add`/`rm`/`suggest --apply` are the last Web-API writes standing, and that is sequencing, not an exception.** They write `dc:relation` item relations — real bibliographic data, and the reason they MOVE rather than retire: a recorded preprint↔published relation is the shape zot's resolver could someday trust as data. zot grows the verbs first, then sci stubs its copies; never a gap with zero homes. **Rule 18 stays at "no upstream index" until that lands** — tightening it to "no `net/http` outside {api reads, connector}" is the job of the ticket that closes the link family, and a fence that lies about what it forbids is worse than a looser honest one.

**No metered third-party index is reachable at all.** `pkg/{openalex,crossref,doiorg}` stay in this repo because the `zot` binary imports them, but nothing under `internal/zot/` does. **`scripts/lint-guard.sh` rule 18 is the fence**, in two halves: `go list -deps ./internal/zot/cli` must not reach `pkg/{openalex,crossref,doiorg}`, and no package under `internal/zot/` outside `{api,cli,client,connector}` may reach `net/http`. It is a dependency check rather than a grep because the failure is transitive — nothing has to import `pkg/openalex` directly for a `sci zot search` to start spending someone's rate limit.
...
- `internal/zot/extract` survives as what sci still needs to READ an extraction's traces: `DoclingTag` / `MarkdownTag` (`link suggest` refuses a docling note; `search --notes` filters on the tag) and `HashPDF` / `ContentKey` (`doctor duplicates`). Nothing in it runs docling.
...
- **`suggest` refuses docling extractions.** Scanning one would walk the *paper's* bibliography — hundreds of references, mostly not in the library, none of them the user's curation. Detected by the `docling` tag, returns `CodeUsage`.
