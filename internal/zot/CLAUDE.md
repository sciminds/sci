...
- **Exported API reads** for the `--remote` CLI flag on `item read`, `item list`, `collection list`. `api.Client.GetItem`, `ListItems`, `ListCollections` are exported methods on `*Client` — **not on the `Writer` interface**. They convert to `local.Item` / `local.Collection` via `api.ItemFromClient` / `CollectionFromClient` so consumers see one uniform shape regardless of source. Prefer `local.Reader` for normal reads (fast, no rate limit); use `--remote` when the local DB is stale or for agent flows that need ground truth.
