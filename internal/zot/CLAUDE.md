...
- **Note-content search** (`local/notes_search.go`, `zot search --note-text`): the ~5k docling extraction notes were invisible to local search (title/DOI/pub/creators/citekey only). `SearchNotes` is the note twin of `SearchFulltext` — same widening path (`SearchOptions.Notes` merges its item IDs into the same set `buildClauseSQL` ORs into positive free-text clauses), child note → parent item, standalone note → itself.
  - **Two stages, and the split is load-bearing.** Notes are stored as HTML, so a bare SQL `LIKE` makes `div`/`class`/`znv1` match the whole library. SQL narrows; Go then strips tags (`NoteText`: tags → *space*, so `<h1>A</h1><p>B` can't weld into `AB`) and re-checks. Verified on the real library: `znv1` / `zotero-note` / `data-schema-version` → 0 hits.
  - **The SQL stage must never under-match**, or stage 2 never sees the row. Two things break a naive LIKE against raw HTML in exactly that direction — entity escaping (`&` is stored `&amp;`) and tag boundaries (`Representations</h1><p>The`). `likeProbe` therefore narrows on the longest fragment split on whitespace *and* on every HTML-escaped char; a word with no usable probe contributes no LIKE and is enforced entirely in Go. `escapeLike` + `ESCAPE '\'` keep `%`/`_` literal.
  - Matching is **substring** (consistent with the `instr`-based metadata search), so short terms hit word interiors — `div` matches `individual`. Worst case on the 5,088-note / 55MB corpus is ~5.5s; typical ~2s.
  - **`--notes` ≠ `--note-text`.** The former filters to items that *have* docling notes; the latter searches *inside* note bodies. Both local-only.
