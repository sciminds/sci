...
- **BibTeX scope**: `exportBibTeX` is intentionally minimal. Reuses Better BibTeX's `citationKey` field when present and does only basic `{` escaping. For full LaTeX escaping users should use Better BibTeX's own export.
