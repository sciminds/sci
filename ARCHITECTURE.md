...
Every `lipgloss.NewStyle()` in the codebase lives in either `internal/ui/` (for shared CLI styling — spinners, prompts, headers) or `internal/tui/<app>/ui/` (for a specific TUI). Code asks for styles via the `ui.TUI` singleton:

```go
fmt.Println(ui.TUI.Header("hello"))
```

The reason is uniformity — every command should look like every other command — and the ability to retheme everything in one place. Inline `lipgloss.NewStyle()` calls outside `ui/` packages are a lint error. `huh` forms (we use them for interactive prompts) plug into `ui.HuhTheme()` and `ui.HuhKeyMap()` for the same reason.
...
- **Styles** — `internal/ui/` for CLI, `internal/tui/<name>/ui/` for that specific TUI.
